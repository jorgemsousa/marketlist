...
const Dashboard = () => {
  ...
  const chartData = closedLists.map(l => ({
    label: l.name,
            ...
            <View style={{ height: 300, width: 'auto', padding: 16 }}>
              {chartData.length > 0 ? (
                ...
                >
                  {({ points, chartBounds }) => (
                    <>
                      <Line points={points.total} color={"#ac24db"} strokeWidth={3} animate={{ type: 'timing', duration: 500 }} />
                      <Area points={points.total} y0={chartBounds.bottom} animate={{ type: 'timing', duration: 500 }} color={"#ac24db"} opacity={0.2}>
                ...
                <Text className="text-center text-gray-500 mt-10">Nenhuma lista finalizada ainda</Text>
              )}
            </View>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View className="flex-row p-4 bg-gray-100 items-center justify-between rounded-lg mt-2 w-full">
                    ...
                    <Text style={{ color: '#7b3fbd', fontWeight: 'bold' }}>{item.total.toFixed(2)}</Text>
                  </View>
                )}
              />
            </View>
